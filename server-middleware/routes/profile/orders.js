...
router.get('/', async (req, res) => {
  ...
  const startData = req.query['startData'] !== "" ? `&DateStart=${req.query['startData'].split('-').join('')}` : ""
  const endData = req.query['endData'] !== "" ?
    `&DateEnd=${req.query['endData'].split('-').join('')}` :
    `&DateEnd=${dateNow().split('-').join('')}`

  const response = await axios(`${api}/GetOrdersByClient/${id}?offset=${offset}&count=${count}${startData}${endData}`, {
...
router.get('/info', async (req, res) => {
  ...
  const response = await axios(`${api}/GetOrderItems/${id}`, {
    auth: {username: username, password: password}
  })
